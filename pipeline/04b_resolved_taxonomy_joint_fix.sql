-- =====================================================================
-- THEOBROMA pipeline stage 04b: genus/family joint-lineage correction
-- =====================================================================
-- Purpose : Rebuild genus/family + full lineage JOINTLY from compound_taxonomy
--           so each compound's ranks come from ONE consistent organism lineage
--           (corrects the independent-per-rank voting that could pair a genus
--           with a family from a different organism, e.g. Gramine Dioscorea +
--           Poaceae). kingdom + secondary_kingdoms left untouched.
-- Inputs  : compound_taxonomy(comp_id, genus, family, phylum_any, class_any,
--           order_any)
-- Output  : corrected resolved_taxonomy(genus, family, taxorder, taxclass, phylum)
-- Run     : after stage 04 (build_resolved_taxonomy_v2). v2 already selects a
--           consistent tuple; this pass corrects residual live inconsistencies.
-- Effect  : ~28,139 rows changed; residual non-co-attested genus/family = 0.
-- Verbatim from fix_genus_family_joint_apply.sql (validated + applied).
-- =====================================================================
BEGIN;
