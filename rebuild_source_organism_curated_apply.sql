-- Rebuild source_organism_curated against the corrected resolved_taxonomy.
-- Same tiering as the original curation (genus-match -> family-match -> rest, alpha within tier).
BEGIN;
WITH exploded AS (
...
SELECT count(*) AS repopulated FROM compounds WHERE source_organism_curated IS NOT NULL;
SELECT 'Gramine now leads with' AS note, split_part(source_organism_curated,'; ',1) FROM compounds WHERE comp_id='THEO_0412143';
COMMIT;
