-- LOTUS-enriched source_organism_curated rebuild (APPLY, COMMITS).
-- Identical logic to the validated dry-run. Only compounds.source_organism_curated
-- changes. Compounds with no clean binomial -> NULL curated (template shows raw).
-- Backups on laptop: source_organism_curated_pre_lotus_merge.csv, compounds_pre_lotus_merge.sql
BEGIN;
