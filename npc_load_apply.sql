-- =====================================================================
-- LOAD the full-corpus NPClassifier re-run and APPLY it to compounds.
-- Input: npc_output.csv (comp_id, np_pathway, np_superclass, np_class)
-- from the canonical GNPS2 API -> uniform, coherent triples for ALL
-- 1,133,004 compounds (fixes the v134-exclusive residual scramble too).
--
-- Staged: load -> coverage check -> archive -> dry-run update -> verify
-- on curcumin + THEO_1055282 -> COMMIT / ROLLBACK.
-- =====================================================================
...
\echo '=== do all live compounds have a row? (should match 1,133,004) ==='
...
-- 1. normalize the multi-value separator to match existing convention.
--    API returns 'A;B'; the DB used ' | ' for pathway and '$'/'; ' variants.
--    Keep it simple: store API's ';'-joined form but convert to ' | ' to match
--    what the app/templates expect for multi-pathway display.
...
--    so an empty/error API row does not wipe an existing value).
UPDATE compounds c
...
-- 6. coherence check: Quassinoids should now be tightly coupled + plant-dominated.
\echo '=== Quassinoids pathway/superclass spread AFTER (should be coherent) ==='
SELECT np_pathway, np_superclass, count(*) FROM compounds
WHERE np_class LIKE '%Quassinoids%' GROUP BY 1,2 ORDER BY 3 DESC LIMIT 8;
\echo '=== Quassinoids by kingdom AFTER (should be overwhelmingly plant) ==='
SELECT kingdom, count(*) FROM compounds WHERE np_class LIKE '%Quassinoids%' GROUP BY 1 ORDER BY 2 DESC;

\echo '=== overall: distinct superclasses per pathway (coherence sanity) ==='
